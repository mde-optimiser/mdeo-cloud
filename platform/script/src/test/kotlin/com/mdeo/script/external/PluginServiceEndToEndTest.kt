...
class PluginServiceEndToEndTest {
    ...
    fun `a call changes the argument and returns a double`() {
        dispatcher("run-token").use { dispatcher ->
            ...
            val total = dispatcher.call("normalize", arrayOf(values))
            ...
            values.add(0.0)
            assertEquals(1.0, dispatcher.call("normalize", arrayOf(values)))
        }
    }
    ...
    fun `a failing operation reaches the script as an error`() {
        dispatcher("run-token").use { dispatcher ->
            val error = assertFailsWith<ExternalCallException> { dispatcher.call("fail", arrayOf()) }
    ...
    fun `a refused token fails the call`() {
        dispatcher("forged").use { dispatcher ->
            assertFailsWith<ExternalCallException> { dispatcher.call("normalize", arrayOf(ListImpl(listOf(1.0)))) }
        }
    }
}
